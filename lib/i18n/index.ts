 ...
 *   4. Default: 'en'
 *
 * Components receive a `t(key)` helper that translates a dotted
...
const en: Dict = {
  ...
  page: {
    ...
    admin_settings: {
      ...
      info: 'How this works',
      info_body:
        'For each closed change, the dashboard compares actual time (open→close) against this baseline. The "ROI" is the percentage of baseline time saved by using baseline. If a dev provides a per-change estimate (via --estimate), that estimate overrides this default for that specific change.',
      save: 'Save baselines',
      reset: 'Reset',
    },
    token_issued: {
  ...
  empty_state: {
    ...
    no_changes_help_long:
      'No changes tracked yet. Have your devs run baseline openspec new <name> --type feature.',
...
const es: Dict = {
  ...
  page: {
    ...
    skills: {
      ...
      no_skills_help: 'El tracking se dispara automáticamente cuando los devs corran baseline install.',
    },
    events: {
      title: 'Eventos',
    ...
    admin_settings: {
      ...
      info: 'Cómo funciona',
      info_body:
        'Por cada change cerrado, el dashboard compara el tiempo real (open→close) contra este baseline. El "ROI" es el porcentaje del baseline que se ahorró usando baseline. Si un dev pasa un estimate per-change (con --estimate), ese estimate override el default para ese change específico.',
  ...
  empty_state: {
    ...
    no_changes_help_long:
      'No hay cambios tracked aún. Que los devs corran baseline openspec new <nombre> --type feature.',
...
export function t(locale: Locale, key: string): string {
  ...
  // Fallback to English
  if (locale !== 'en') {
    const enStr = resolveDict(DICTIONARIES.en, key);
    if (enStr !== key) return enStr;
  }
  return key;
}

/**
 * Get the user's preferred locale from a request-like object.
