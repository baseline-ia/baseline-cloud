...
import { users, tokens, sessions, auditLog } from '../db/schema.js';
...
export async function resolveBearerToken(raw: string): Promise<ResolvedToken | null> {
  ...
  // Use .execute() for postgres (no `.run()` like sqlite).
  void db
    .update(tokens)
...
export async function createSession(userId: string, ip?: string): Promise<{ session: DashboardSession; cookieValue: string }> {
  const id = nanoid(32);
...
function verifyCookieValue(value: string): CookiePayload | null {
  ...
  try {
    ...
    if (typeof payload.id !== 'string' || typeof payload.userId !== 'string' || typeof payload.sessionSecret !== 'string' || typeof payload.expiresAt !== 'number') {
      return null;
    }
...
export async function resolveSession(cookieValue: string | undefined): Promise<DashboardSession | null> {
  ...
  const rows = await db
    ...
    .where(and(eq(sessions.id, payload.id), eq(sessions.sessionHash, sessionHash), gt(sessions.expiresAt, new Date())))
    .limit(1);
